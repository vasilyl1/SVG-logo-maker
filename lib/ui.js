...
function userInterview(fileName, path, data) { // interview the user and generate the file content
    inq.prompt([
        ...
        .then((response) => {
            ...
            shape.shapeColor = colorString.to.rgb(colorString.get.rgb(response.shapeColor)); //set up shapecolor, textcolor is inherited from Shapes
            shape.text = response.logoText;
            shape.textColor = colorString.to.rgb(colorString.get.rgb(response.textColor));
            shape.setColor(); // write colors info into the SVG string, in Shapes
            shape.render();
